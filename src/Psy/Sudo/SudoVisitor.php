...
class SudoVisitor extends NodeVisitorAbstract
{
    ...
    public function enterNode(Node $node)
    {
        if ($node instanceof PropertyFetch) {
            ...
            return new StaticCall(new Name\FullyQualified(self::SUDO_CLASS), self::METHOD_CALL, $args);
            ...
            // not using prepareCall because the $node->args we started with are already Arg instances
            return new StaticCall(new Name\FullyQualified(self::SUDO_CLASS), self::STATIC_CALL, $args);
    ...
    {
        return new StaticCall(new Name\FullyQualified(self::SUDO_CLASS), $method, array_map(function ($arg) {
            return new Arg($arg);
        }, $args));
    }
}
